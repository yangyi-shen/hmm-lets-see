...
<template>
  <main>
    <section>
      <p class="text-2xl font-bold mb-1">Hmm, let's see... 🤔</p>
      <p>
    ...
    <section>
      <form class="flex flex-col gap-2 mt-8 mb-4" @submit.prevent="fetchAnalysis()">
        <input
          v-model="prompt"
          class="placeholder:text-slate-500 border-1 border-slate-950 rounded px-2"
          type="text"
          placeholder="How long can people smoke cigarettes before the health effects become evident?"
          required
        />
        <button v-if="!loading" type="submit" class="bg-amber-500 text-white font-semibold px-2 rounded hover:cursor-pointer">Ask</button>
        <button v-else class="bg-amber-500 opacity-50 text-white font-semibold px-2 rounded" disabled>Loading...</button>
      </form>
    </section>
